    ...
    <div class="container-fluid">
        <div class="row justify-content-evenly m-0 mt-3 mb-3">
            <div class="card-header col-xl-6">
                <h5 class="card-title mb-0">Thêm mới xe máy</h5>
            </div>
            ...
            <div class="col-xl-6">
                <div class="mb-3">
                    <label for="tenXe" class="form-label"
                        >Tên xe<span class="text-danger">*</span></label
                    >
...
<script lang="ts">
    ...
    import { addXeMay } from '@/services/xeMayService'
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            onMounted(async () => {
                await Promise.all([fetchDanhMucXes()])
            ...
            const handleAnhXeMayChange = (event: Event) => {
                ...
                previewAnhXeMays.value.forEach((url) => URL.revokeObjectURL(url))
                previewAnhXeMays.value = []
                selectedAnhXeMays.value = Array.from(files)

                for (const file of selectedAnhXeMays.value) {
            ...
            const removeAnhXeMay = (index: number) => {
                URL.revokeObjectURL(previewAnhXeMays.value[index])

                previewAnhXeMays.value.splice(index, 1)
                selectedAnhXeMays.value.splice(index, 1)
            ...
            const handleThemMoi = async () => {
                ...
                try {
                    ...
                    formData.append('maChuCuaHang', getMaNguoiDung())

                    selectedAnhXeMays.value.forEach((file) => {
                        formData.append('anhXeMayList', file)
                    })

                    const response = await addXeMay(formData)
