        ...
        <div class="border-start border-dark-subtle">
            <div class="row justify-content-evenly m-0 mt-3 mb-3 card-header">
                ...
                <div class="card-header col-6 text-end">
                    <router-link class="text-success" :to="{ name: 'DanhSachKhieuNaiView' }">
                ...
                <div class="row">
                    ...
                    <div class="col-6 b-3">
                        ...
                        <textarea
                            ...
                            id="noiDungPhanHoi"
                            rows="5"
                            :disabled="khieuNai.trangThaiXuLy === 'Đã xử lý'"
                        ></textarea>
                    </div>
                </div>
                <div class="mt-3 mb-3" v-if="khieuNai.trangThaiXuLy === 'Chờ xử lý'">
                    <div class="text-center">
                        <button
                            type="button"
                            class="btn btn-primary"
                            title="Phản hồi"
                            @click.prevent="handlePhanHoi"
                            :disabled="isLoading"
                        >
                            Phản hồi
                        </button>
...
<script lang="ts">
    ...
    import { getKhieuNaiByMaKhieuNai, updateKhieuNai } from '@/services/khieuNaiService'
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const handlePhanHoi = async () => {
                if (!khieuNai.value.noiDungPhanHoi) {
                    isError.value = true
                ...
                try {
                    ...
                    formData.append('noiDungPhanHoi', khieuNai.value.noiDungPhanHoi)
                    formData.append('ngayPhanHoi', getLocalDateTimeString())
                    formData.append('trangThaiXuLy', 'Đã xử lý')

                    const response = await updateKhieuNai(
                        String(khieuNai.value.maKhieuNai),
                        formData
                    )
                    if (response.success) {
                        await router.push({ name: 'DanhSachKhieuNaiView' })
                    }
                } catch (error) {
